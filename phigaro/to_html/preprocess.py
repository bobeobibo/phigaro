...
def form_arrow(begin, end, nonreverse, width):
    ...
    if nonreverse:
        return (
            (begin, 1 - width * k),
            (begin, 1 + width * k),
            (begin + 0.6 * (end - begin), 1 + width * k),
            (begin + 0.6 * (end - begin), 1 + 1.8 * height * k),
            (end, 1),
            (begin + 0.6 * (end - begin), 1 - 1.8 * height * k),
            (begin + 0.6 * (end - begin), 1 - width * k),
        )
        ...
        return (
            (end, 1 - width * k),
            (end, 1 + width * k),
            (end - 0.4 * (end - begin), 1 + width * k),
            (end - 0.4 * (end - begin), 1 + 1.8 * height * k),
            (begin, 1),
            (end - 0.4 * (end - begin), 1 - 1.8 * height * k),
            (end - 0.4 * (end - begin), 1 - width * k),
        )
    ...
    def _plotly_to_html_(
        ...
        layout = {
            ...
            'shapes': [
                ...
                for group, elements in func_groups.items()
                for arrow in np.array(arrow_coords)[elements]
            ],
        }
        data = traces0 + trace1
        fig = {
